...
from model.DDItransformer import TransformerDDI_Sequence
import argparse
...
config=DrugConfig(num_hidden_layers=num_encoders,dropout_prob=dropout)
model=TransformerDDI_Sequence(config)

#Here Datasets are defined
training_dataset=DDIdatasetSequence(training_csv,vocabulary,config=config)
validation_dataset=DDIdatasetSequence(validation_csv,vocabulary,config=config)
...
if __name__=='__main__':
    ...
    for epoch in range(0, N_EPOCH):
        ...
        for bathc_id, (drug1_ID,drug2_ID,drug_token_id,drug_mask,drug_sentence_mask,label) in enumerate(training_loader):
            drug_token_id,drug_mask,drug_sentence_mask,label=\
                    drug_token_id.to(device),drug_mask.to(device),\
                    drug_sentence_mask.to(device),label.to(device)

            #Set the gradient to 0
            opt.zero_grad()

            #Forward-pass
            label_pred=model(drug_token_id,drug_mask,drug_sentence_mask)
        ...
        for drug1_ID,drug2_ID,drug_token_id,drug_mask,drug_sentence_mask,label in validation_loader:
