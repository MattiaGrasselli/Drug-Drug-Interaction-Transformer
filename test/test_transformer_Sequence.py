...
parser=argparse.ArgumentParser(description="Drug-Drug Interaction test for TransformerDDI Sequence")
...
config=DrugConfig(vocabulary_size=73,max_position_embedding=256,num_hidden_layers=num_encoders,dropout_prob=dropout)
...
test_dataset=DDIdatasetSequence(test_csv,vocabulary,config)
...
if __name__=='__main__':
    ...
    for bathc_id, (drug1_ID,drug2_ID,drug_token_id,drug_mask,drug_sentence_mask, label) in enumerate(test_loader):
        ...
        with torch.no_grad():
            drug_token_id,drug_mask,drug_sentence_mask, label=\
                drug_token_id.to(device),drug_mask.to(device), drug_sentence_mask.to(device), label.to(device)
